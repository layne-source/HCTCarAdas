...
| 结论定性 | 链路骨架健康，无方向性缺陷；**当前未闭环 1 项 P0 + 2 项 P1 + 1 组 P2**（P2-8 需产品决策，P0-1 等实测，P1-2 需设备验证） |
| 修订记录 | **rev.2**：撤销 2 项误判、重定性 2 项为产品权衡<br>**rev.3**：基线 `6599789`，闭环 3 项 P1 + 2 组 P2，新增 P2-10<br>**rev.4**：基线 `32ea041`，闭环 P0-2 与 P2-10、新增 P2-11<br>**rev.5**：基线 `c6c1b46`，闭环 7 组 P2，本级别仅剩 P2-8；详见第 0 节 |
...
> **rev.5 需你确认的两点**：
> 1. **P2-4 附带行为变更**：未记录相机 ID 的旧标定现在会被判定为"无法归属"并失效一次，车辆需重标定一次。取舍理由见第 6 节 P2-4。
> 2. **P2-8 保持开放**：息屏是否可接受静默降级属产品决策，未擅自加 WakeLock 或前台 Service。
...
需要排期处理的问题共 **1 项 P0 + 2 项 P1 + 1 组 P2**（rev.5 口径；另有 2 项属产品策略权衡而非缺陷，依据见第 4.2 节）。三项剩余的共同点是**都不靠改代码解决**：P0-1 等设备实测数据、P1-2 等设备验证、P2-8 需产品决策。rev.2 时的 2 + 5 + 9 已在 `6599789`、`21679fa`、`8b733a0`、`c6c1b46` 四个提交中基本清空，明细见第 0 节与第 6 节。
...
rev.5 对 `ee79474`、`8b733a0`、`c6c1b46` 三个提交逐文件复核后，**P2 级别除 P2-8 外全部闭环**，逐项依据见第 0 节与第 6 节。代码侧自查结论：
...
**唯一需要你确认的行为变更**：P2-4 使"未记录相机 ID 的旧标定"失效一次（第 6 节 P2-4 有取舍说明）。
...
## 4. P0 — 影响预警正确性或安全延迟（rev.5：1 项待实测）
...
## 6. P2 — 可维护性与一致性（rev.5：除 P2-8 外全部闭环）

> **rev.5 状态**：P2-2、P2-4、P2-5、P2-6、P2-7、P2-9、P2-11 随 `8b733a0` / `c6c1b46` 一次性闭环。叠加此前已闭环的 P2-1、P2-3、P2-10，**本级别仅剩 P2-8**（无 WakeLock / 前台 Service，息屏可能静默降级），该项需产品决策而非代码修复。
>
...
**⚠️ 行为变更（需实车确认）**：早期版本保存的、未记录 camera id 的标定现在会被失效一次，车辆需要重新标定一次。这是有意的取舍——"无 ID"无法证明该标定属于当前摄像头，继续沿用会让"同分辨率换摄像头继承旧 pitch"这一缺陷保留。若产品上不能接受这次重标定，应改为一次性迁移（补写当时的 camera id）而不是保留放行。
...
**P2 处理优先序**（rev.5）：**可立即处理项已全部完成**（P2-2、P2-4、P2-5、P2-6、P2-7、P2-9、P2-11）。剩余仅：

| 项 | 状态 |
| --- | --- |
| P2-8 WakeLock / 前台 Service | ⏳ 需产品决策（是否接受息屏静默降级） |
| P0-1 延迟收口 | 🔶 等实测日志 |
| P1-2 USB 广播可达性 | 🔎 需设备验证 |
| P2-4 附带的行为变更 | ⚠️ 需实车确认"旧标定失效一次"可接受 |
...
**可立即改代码的项已全部清零。** 剩余项的共同点是都需要本报告之外的信息：

| 序 | 项 | 阻塞于 | 说明 |
| --- | --- | --- | --- |
| 1 | **P0-1 延迟收口**（帧数 3→2 + 时间窗 300–400 ms） | 设备实测日志 | 仪表已就位；**先读 `[DANGER-START]` / `Confirm` 再改**，用推算值调报警阈值不负责 |
| 2 | P1-2 USB 广播可达性 | 设备三条授权路径 | 静态审查无法判定 |
| 3 | P2-8 WakeLock / 前台 Service | 产品决策 | 是否接受息屏静默降级 |
| 4 | P2-4 附带行为变更确认 | 产品确认 | 旧标定失效一次是否可接受 |
| 5 | 标定参数实车标定 | 真实车道帧 | `MIN_ROI_FAR_DISTANCE_METERS`、`MAX_CONVERGENCE_STD_DEV`、提示阈值 5 次 |
...
> **rev.5 相比 rev.4**：待办从 9 项降到 **5 项，且全部不是"写代码就能收"的项**。代码侧审查结论收敛：链路骨架健康，无方向性缺陷，报警逻辑与降级策略均可辩护。
...
3. **换摄像头后的标定失效行为**：用两台同分辨率 USB 摄像头交替接入，确认第二次接入时旧标定被失效并提示重新标定（`reloadCalibrationForCamera`）—— 对应 P2-4
4. 破坏一个 `alerts/*.wav` 后启动，确认 UI 出现"报警音不可用"而不是仍显示 READY —— 对应 P2-2
5. 息屏 / 后台 30 min 长稳运行，记录分析率与漏报 —— 对应 P2-8
6. 1280×720 → 640×480 降级路径复测：叠加层车道已被门控，确认降级时不再出现错位车道、参考线正常提示 —— 对应已闭环 P1-2 的回归
