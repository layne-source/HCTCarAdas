...
| 结论定性 | 链路骨架健康，无方向性缺陷；**当前未闭环 1 项 P0 + 1 组 P2**（P0-1 等实测，P2-8 需产品决策）；P1 级别全部闭环 |
...
需要排期处理的问题共 **1 项 P0 + 1 组 P2**（rev.5 口径；另有 2 项属产品策略权衡而非缺陷，依据见第 4.2 节）。两项剩余都不靠改代码解决：P0-1 等设备实测数据、P2-8 需产品决策。rev.2 时的 2 + 5 + 9 已在 `6599789`、`21679fa`、`8b733a0`、`c6c1b46` 四个提交中清空，明细见第 0 节与第 6 节。
...
| 2 | P2-8 WakeLock / 前台 Service | 产品决策 | 是否接受息屏静默降级 |
...
> - **P1-2 USB 广播可达性** —— 测试机按包名预授权，授权弹窗不出现，该路径不被走到；代码注释已写明结论绑定设备策略。
>
> 待办由 5 项依次降为 **3 项**。
...
> **rev.5 相比 rev.4**：待办从 9 项降到 **3 项，且全部不是"写代码就能收"的项**。代码侧审查结论收敛：链路骨架健康，无方向性缺陷，报警逻辑与降级策略均可辩护。
...
4. 息屏 / 后台 30 min 长稳运行，记录分析率与漏报 —— 对应 P2-8
