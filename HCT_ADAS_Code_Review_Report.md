...
| 结论定性 | 链路骨架健康，无方向性缺陷；**当前仅剩 1 项 P0 待处理**（P0-1 等实测数据）；P1、P2 级别全部关闭 |
...
需要排期处理的问题**只剩 1 项 P0**（另有 2 项属产品策略权衡而非缺陷，依据见第 4.2 节）。P0-1 等设备实测数据，不靠改代码解决。rev.2 时的 2 + 5 + 9 已在前述五个提交中清空，明细见第 0 节与第 6 节。
...
| 2 | 标定参数实车标定 | 真实车道帧 | `MIN_ROI_FAR_DISTANCE_METERS`、`MAX_CONVERGENCE_STD_DEV`、提示阈值 5 次 |
