...
> **口径严谨性说明**：Age 是从**采集时刻**到决策时刻的差，其最小值（94 ms）包含"当次剩余的节流等待 + 排队 + 单帧处理"。因此 94 ms 是**测量段的下界**、也是线程占用的下界——真实的单帧处理时间可能略低于 94 ms（当剩余节流等待接近 0 时二者相等）。
>
> **rev.7 已补分段指标**：`Frame` 现在携带 `offeredAtNanos`（生产者交接时刻），`FrameConsumer.Metrics` 增加 `offeredNanos` / `pickedUpNanos` / `finishedNanos`，心跳日志输出 `Split=采集→交接+排队+处理 ms`。三个分量都是同钟差、**不含 250 ms 心跳相位**，可直接回答"延迟该归到节流、队列还是管线"——这也是评估"车道检测降频能省多少"所需的数据。注意 `Age` 与 `Split` 的合计相等（`Age = 采集→交接 + 排队 + 处理 + 交接后到输出`），因此两者互为交叉校验。
